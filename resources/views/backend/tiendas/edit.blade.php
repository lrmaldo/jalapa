<x-app-layout>
    ...
    <script>
        let latitude = {!!json_encode($tienda->latitude) !!};
        let longitude = {!!json_encode($tienda->longitude)!!};
      ...
      let  mapa = {
            ...
            initMap: function() {
                ...
                mapa.marker = new google.maps.Marker({
                    position: {
                        lat: latitude?parseFloat(latitude) : 18.0709008,
                        lng: longitude? parseFloat(longitude): -96.5349083
                    },
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="post" action="{{ route('tiendas.update',$tienda->id) }}" enctype ='multipart/form-data'>
                    @method('PUT')
                    @csrf
                    <div class="shadow overflow-hidden sm:rounded-md">
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="checkbox" name="is_active" id="is_active" value="1" {{$tienda->is_active?'checked': null}} >
                        </div>
                        <div class="px-4 py-5 bg-white sm:p-6">
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            <label for="logo_url" class="block font-medium text-sm text-gray-700">Logo</label>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="hidden" id="lat" class="form-control" name="lat" value="{{$tienda->latitude?$tienda->latitude:"18.0864363"}}"
                                placeholder="" required>
                            <input type="hidden" id="long" class="form-control" name="long" value="{{$tienda->longitude ? $tienda->longitude : "-96.1248874" }}"
                                placeholder="" required>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select name="categoria_id" id="categoria_id" class="form-multiselect  rounded-md shadow-sm mt-1 block w-full" >
                                ...
                                @foreach($categorias as  $categoria)
                                    <option value="{{ $categoria->id }}"{{  old('categoria_id', $tienda->categoria_id) ? ' selected' : '' }}>{{ $categoria->nombre }}</option>
                                @endforeach
                            </select>
                            @error('categoria_id')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select name="tipo_tienda" id="tipo_tienda" class="form-multiselect  rounded-md shadow-sm mt-1 block w-full" >
                                @php
                                    $array_option = ['Directorio',"Tienda con productos"]
                                @endphp
                                <option disabled selected>Seleccionar</option>
                                @foreach($array_option as $key => $value)
                                <option value="{{$key+1}}" {{$key+1 == $tienda->tipo_tienda ? 'selected':''}}>{{$value}}</option>
                                @endforeach
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="text" name="facebook_url" id="facebook_url" placeholder="https://www.facebook.com/XXXXXXXX" class="form-control rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('facebook_url', $tienda->facebook_url) }}" />
                            @error('facebook_url')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <textarea name="descripcion" id="descripcion" class="form-control rounded-md shadow-sm mt-1 block w-full" placeholder="Escribe una descripción de la tienda">{{old('descripcion',$tienda->descripcion)}}</textarea>
                            @error('ip')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
